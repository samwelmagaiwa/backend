...
use App\Http\Requests\UserInternetAccessFormRequest;
...
class UserInternetAccessFormController extends Controller
{
    /**
    ...
    /**
     * Submit a new Internet access request from user form.
     */
    public function store(UserInternetAccessFormRequest $request): JsonResponse
    {
        DB::beginTransaction();
        ...
        try {
            ...
            if (!$this->hasStaffRole($user)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Only staff members can submit Internet access requests.',
            ...
            if (!$signaturePath) {
                return response()->json([
                    'success' => false,
                    'message' => 'Digital signature is required for Internet access requests.',
                    'error' => 'Missing signature'
                ], 422);
            }

            // Create the Internet access request
            ...
            Log::info("Internet access request created", [
                'user_id' => $user->id,
                'pf_number' => $validatedData['pf_number'],
                'request_id' => $userAccess->id,
                'department_id' => $validatedData['department_id'],
                'purpose_length' => strlen($validatedData['purpose'])
            ]);
            ...
            return response()->json([
                'success' => true,
                'data' => $userAccess,
                'message' => 'Internet access request submitted successfully. Your request is now under review.',
                'request_id' => $userAccess->id,
                'next_steps' => [
            ...
            Log::error('Error creating Internet access request: ' . $e->getMessage(), [
                'user_id' => Auth::id(),
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to submit Internet access request. Please try again.',
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'data' => $requests,
                'message' => 'Internet access requests retrieved successfully.'
            ]);

        } catch (\Exception $e) {
            Log::error('Error retrieving Internet access requests: ' . $e->getMessage());
            
            return response()->json([
                'success' => false,
                'message' => 'Failed to retrieve Internet access requests.',
                'error' => config('app.debug') ? $e->getMessage() : 'Internal server error'
            ], 500);
        }
    }
    ...
    /**
     * Get a specific Internet access request.
     */
    public function show(UserAccess $userAccess): JsonResponse
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'data' => $userAccess,
                'message' => 'Internet access request retrieved successfully.'
            ]);

        } catch (\Exception $e) {
            Log::error('Error retrieving Internet access request: ' . $e->getMessage());
            
            return response()->json([
                'success' => false,
                'message' => 'Failed to retrieve Internet access request.',
                'error' => config('app.debug') ? $e->getMessage() : 'Internal server error'
            ], 500);
        }
    }
