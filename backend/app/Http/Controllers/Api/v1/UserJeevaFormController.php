...
use App\Http\Requests\UserJeevaFormRequest;
...
class UserJeevaFormController extends Controller
{
    /**
    ...
    /**
     * Submit a new Jeeva access request from user form.
     */
    public function store(UserJeevaFormRequest $request): JsonResponse
    {
        DB::beginTransaction();
        ...
        try {
            ...
            if (!$this->hasStaffRole($user)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Only staff members can submit Jeeva access requests.',
            ...
            if (!$signaturePath) {
                return response()->json([
                    'success' => false,
                    'message' => 'Digital signature is required for Jeeva access requests.',
                    'error' => 'Missing signature'
                ], 422);
            }

            // Create the Jeeva access request
            ...
            Log::info("Jeeva access request created", [
                'user_id' => $user->id,
                'pf_number' => $validatedData['pf_number'],
            ...
            return response()->json([
                'success' => true,
                'data' => $userAccess,
                'message' => 'Jeeva access request submitted successfully. Your request is now under review.',
                'request_id' => $userAccess->id,
                'next_steps' => [
            ...
            Log::error('Error creating Jeeva access request: ' . $e->getMessage(), [
                'user_id' => Auth::id(),
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to submit Jeeva access request. Please try again.',
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'data' => $requests,
                'message' => 'Jeeva access requests retrieved successfully.'
            ]);

        } catch (\Exception $e) {
            Log::error('Error retrieving Jeeva access requests: ' . $e->getMessage());
            
            return response()->json([
                'success' => false,
                'message' => 'Failed to retrieve Jeeva access requests.',
                'error' => config('app.debug') ? $e->getMessage() : 'Internal server error'
            ], 500);
        }
    }
    ...
    public function show(UserAccess $userAccess): JsonResponse
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'data' => $userAccess,
                'message' => 'Jeeva access request retrieved successfully.'
            ]);

        } catch (\Exception $e) {
            Log::error('Error retrieving Jeeva access request: ' . $e->getMessage());
            
            return response()->json([
                'success' => false,
                'message' => 'Failed to retrieve Jeeva access request.',
                'error' => config('app.debug') ? $e->getMessage() : 'Internal server error'
            ], 500);
        }
    }
