...
export default {
  ...
  computed: {
    ...
    availableRoles() {
      return this.allRoles.filter((role) => {
        ...
        if (role.is_system_role && !this.user.roles.includes('admin')) {
          return false
        }
        return true
      })
  ...
  methods: {
    ...
    async createUser() {
      ...
      try {
        const result = await userRoleService.createUser(this.createUserForm)

        if (result.success) {
          this.showSuccessMessage(result.message || 'User created successfully!')
          ...
          if (result.errors) {
            this.createUserFormErrors = result.errors
          }
        }
      } catch (error) {
    ...
    async fetchDepartments() {
      try {
        const result = await userRoleService.getDepartments()
