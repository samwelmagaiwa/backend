              ...
              >
                ...
                <div class="flex flex-col md:flex-row gap-4 items-center">
                  ...
                  <div class="flex gap-4">
                    ...
                    >
                      <option value="" class="bg-blue-800 text-white">
                        All Status
                      </option>
                      <option value="pending" class="bg-blue-800 text-white">
                        Pending
                      </option>
                      <option value="approved" class="bg-blue-800 text-white">
                        Approved
                      </option>
                      <option value="rejected" class="bg-blue-800 text-white">
                        Rejected
                      </option>
                      <option
                        value="in_progress"
                    ...
                    >
                      ...
                      <option value="jeeva" class="bg-blue-800 text-white">
                        Jeeva Access
                      </option>
              ...
              >
                <div class="p-6 border-b border-blue-300/30">
                  ...
                  <p class="text-sm text-blue-300 mt-2">
                    ...
                    Staff requests displayed in FIFO order. Click "View &
                    Process" to capture: Module Requested for, Module Request,
                    Access Rights, and Comments.
                  </p>
                </div>
                      ...
                      >
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                          <div class="flex items-center">
                            ...
                            >
                              <i
                                :class="getTypeIcon(request.type)"
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                          ...
                          >
                            <i
                              :class="getStatusIcon(request.currentStatus)"
                              class="mr-2"
                            ></i>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-center">
                          ...
                          <div class="text-xs text-blue-300 mt-1">
                            Click to capture: Module Requested for, Module
                            Request, Access Rights, Comments
                          </div>
                        </td>
                      </tr>
                    </tbody>
  ...
  setup() {
    ...
    const filteredRequests = computed(() => {
        ...
        switch (userRole.value) {
          ...
          case ROLES.DIVISIONAL_DIRECTOR:
            return (
              ...
              request.divisionalStatus === 'pending'
            )
      ...
      if (statusFilter.value) {
        filtered = filtered.filter(
          (request) => request.currentStatus === statusFilter.value
        )
      }
      ...
      if (typeFilter.value) {
        filtered = filtered.filter(
          (request) => request.type === typeFilter.value
        )
      }
    ...
    const stats = computed(() => {
      ...
      return {
        pending: roleFilteredRequests.filter(
          (r) => r.currentStatus === 'pending'
        ).length,
        approved: roleFilteredRequests.filter(
          (r) => r.currentStatus === 'approved'
        ).length,
        rejected: roleFilteredRequests.filter(
    ...
    const getApprovalStatusClass = (status, roleType) => {
      const isCurrentUserRole =
        (roleType === 'head_of_department' &&
          userRole.value === ROLES.HEAD_OF_DEPARTMENT) ||
        (roleType === 'divisional_director' &&
    ...
    const getCurrentApprovalStage = (request) => {
      ...
      if (request.divisionalStatus === 'pending')
        return 'Awaiting Divisional Director'
      ...
      try {
        ...
        if (result.success) {
          ...
          requests.value = result.data.map((request) => {
            const requestTypes = Array.isArray(
    ...
    const loadPersonalInfoFromUserAccess = async(userAccessId) => {
      try {
        ...
        const result = await personalInfoService.getPersonalInfoFromUserAccess(
          userAccessId
        )
    ...
    const viewRequestWithPersonalInfo = async(request) => {
      ...
      try {
        ...
        if (request.userAccessId) {
          try {
            const personalInfo = await loadPersonalInfoFromUserAccess(
              request.userAccessId
            )
            ...
            console.warn(
              'Could not load personal info, proceeding without it:',
              error
            )
          }
        }
