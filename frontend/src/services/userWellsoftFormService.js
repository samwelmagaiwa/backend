...
export const userWellsoftFormService = {
  /**
   * Submit Wellsoft access request
   */
  async submitRequest(formData) {
    try {
      const response = await apiClient.post('/user-wellsoft/submit', formData, {
        headers: {
      ...
      console.error('Error submitting Wellsoft request:', error)
  ...
  /**
   * Get user's Wellsoft requests
   */
  async getUserRequests(params = {}) {
    try {
      const response = await apiClient.get('/user-wellsoft/requests', { params })
      ...
      console.error('Error fetching Wellsoft requests:', error)
      throw {
  ...
  /**
   * Get specific Wellsoft request
   */
  async getRequest(requestId) {
    try {
      const response = await apiClient.get(`/user-wellsoft/requests/${requestId}`)
      ...
      console.error('Error fetching Wellsoft request:', error)
      throw {
  ...
  async getDepartments() {
    try {
      const response = await apiClient.get('/user-wellsoft/departments')
  ...
  async checkSignature(pfNumber) {
    try {
      const response = await apiClient.post('/user-wellsoft/check-signature', {
        pf_number: pfNumber
      })
  ...
  formatRequestData(request) {
    return {
      ...
      formatted_request_type: 'Wellsoft Access'
    }
  },
